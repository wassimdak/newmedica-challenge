...
async function seedIfEmpty() {
  ...
  for (const marque of marques) {
    for (let i = 1; i <= 3; i++) {
      ...
      await run('INSERT INTO produits (id, reference, marque_id, unite_vente) VALUES (?, ?, ?, ?)', [id, ref, marque.id, 'unité']);
  ...
  await run(
    `INSERT INTO challenges (id, nom, type, periode_debut, periode_fin, perimetre, marque_id, objectif_unites, regles_points, recompense_associee, actif)
     VALUES (?, ?, 'defi', ?, ?, 'national', ?, 15, ?, ?, 1)`,
    [challengeDefi, 'Lancement Rilastil Sun', debutMois, finMois, rilastilId,
      '3 points par unité Rilastil Sun vendue pendant le lancement.',
  ...
  for (const prep of preparatrices) {
    ...
    await run(
      `INSERT INTO participations (id, challenge_id, preparatrice_id, progression, statut) VALUES (?, ?, ?, ?, ?)`,
      [uuid(), challengeBabe, prep.id, progressionBabe.total, progressionBabe.total >= 40 ? 'reussi' : 'en_cours']
    );

    const progressionMission = Math.min(10, rand(0, 12));
    await run(
      `INSERT INTO participations (id, challenge_id, preparatrice_id, progression, statut) VALUES (?, ?, ?, ?, ?)`,
      [uuid(), challengeMission, prep.id, progressionMission, progressionMission >= 10 ? 'reussi' : 'en_cours']
    );

    const progressionDefi = rand(0, 18);
    await run(
      `INSERT INTO participations (id, challenge_id, preparatrice_id, progression, statut) VALUES (?, ?, ?, ?, ?)`,
      [uuid(), challengeDefi, prep.id, progressionDefi, progressionDefi >= 15 ? 'reussi' : 'en_cours']
    );
  }
  ...
  const adminEmail = process.env.ADMIN_EMAIL || 'admin@newmedica.tn';
  ...
  await run(
    ...
    [uuid(), 'Administrateur NewMedica', adminEmail, await bcrypt.hash(adminPassword, 10)]
  );
  await run(
  ...
  console.log(`Connexion back-office (démo) : ${adminEmail} / ${adminPassword}`);
}
