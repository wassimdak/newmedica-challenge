...
const SESSION_TTL_REMEMBER_MS = 30 * 24 * 60 * 60 * 1000; // 30 jours avec "Se souvenir de moi"
...
function createSession(userId, role, regionId = null, impersonatedBy = null, rememberMe = false) {
  const token = crypto.randomBytes(24).toString('hex');
  const ttl = rememberMe ? SESSION_TTL_REMEMBER_MS : SESSION_TTL_MS;
  sessions.set(token, { userId, role, regionId, impersonatedBy, expiresAt: Date.now() + ttl });
  return { token, ttl };
}
