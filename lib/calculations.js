...
// - chaque unité vendue: points_par_unite de la marque
...
async function recalculerPointsVentes(preparatriceId, periode) {
  ...
  const ventes = await all(
    `SELECT v.marque_id, m.points_par_unite, m.nom AS marque_nom, SUM(v.quantite) AS qte
     FROM ventes v JOIN marques m ON m.id = v.marque_id
     WHERE v.preparatrice_id = ? AND v.periode = ? AND v.statut_validation = 'valide'
     GROUP BY v.marque_id`,
    [preparatriceId, periode]
  );

  for (const v of ventes) {
    const pts = v.qte * v.points_par_unite;
    if (pts > 0) {
      await run(
        `INSERT INTO scores (id, preparatrice_id, montant, motif, periode) VALUES (?, ?, ?, ?, ?)`,
        [uuid(), preparatriceId, pts, `Vente ${v.marque_nom} (${v.qte} unités)`, periode]
      );
    }
  }
...
async function recalcParticipations(periode) {
  ...
  for (const challenge of challenges) {
    for (const prep of preparatrices) {
      ...
      const existing = await get('SELECT id FROM participations WHERE challenge_id = ? AND preparatrice_id = ?', [
        challenge.id, prep.id,
      ]);
      if (existing) {
        await run('UPDATE participations SET progression = ?, statut = ? WHERE id = ?', [progression, statut, existing.id]);
      } else {
        await run(
          'INSERT INTO participations (id, challenge_id, preparatrice_id, progression, statut) VALUES (?, ?, ?, ?, ?)',
          [uuid(), challenge.id, prep.id, progression, statut]
        );
      }
    }
  }
